...
但是，在 [1.14.4-28.0.17](https://github.com/MinecraftForge/MinecraftForge/commit/0bdc2d04b4bef9eb19e01d22720f19240a240241) 中，cpw将自己为Forge编写的一个`ITransformationService`使用JavaScript进行重写，代表着cpw对使用JavaScript编写CoreMod是有一定程度的认可的。  
...
|操作风险|需要开发者特别注意不能在CoreMod阶段调用任何原版类和其他Mod，提前加载这些类会导致错误|不会导致类提前加载|
...
```
* 由于JavaScript的语言特性，全等于（===）与Java的等于（==）比较类似，在比较`String`时全等于与`equals`较为类似
