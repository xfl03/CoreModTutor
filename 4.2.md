...
* 1.8 Forge与FML变更了包名，从`cpw`改成了`net.minecraftforge`
...
(以下代码适用于1.8-1.12.2，1.6.2-1.7.10需要变更import中的FML包名)
...
聪明的读者可能会注意到，`jar`是会在`build`等task打包时才会被执行，在`runClient`之类的测试运行时并不会被读取到，如果需要在测试运行时使用CoreMod，还需要在`build.gradle`中加入jvm参数的设置：
```gradle
minecraft {
    ...
...
* 切记无论如何都要返回一个有效的`byte`数组，否则会导致`ClassNotFoundException`、`NoClassDefFoundError`等崩溃
